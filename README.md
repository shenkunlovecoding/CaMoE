...
1. **RWKV-7 ClampW Kernel 兼容性修复**
- 对齐 `torch.ops.rwkv7_clampw` 固定 namespace 调用方式。
- 修复 CUDA/C++ 宏不一致导致的符号不匹配问题（`undefined symbol`）。
- 增加 kernel 输入 dtype 与 contiguous 断言，避免静默错读内存。
- 增加 `diag` 开关（`baseline / no_amp / no_fast_math / fp32_kernel`）方便定位问题。

2. **训练稳定性修复（0.1b/0.4b_toy）**
- 增加梯度检查点开关：`use_gradient_checkpoint`，可按配置关闭。
- 增加逐层 NaN 定位日志（`nan_debug`），可定位到 block 与子张量。
- 增加 TimeMix 输出保护开关（诊断模式用）。

3. **Loss 口径修复（关键）**
- padding token 统一使用 `-100`，并在 train/eval 的 CE 中使用 `ignore_index=-100`。
- 恢复 tied embedding 输出缩放 `x * (n_embd ** -0.5)`，避免 logits 尺度异常。
- 修复后 loss 数值回到可解释区间（不再出现“几百级”的失真 CE）。

> 说明：如果你在云端看到 “能跑但文本质量怪异”，优先检查是否从随机初始化训练、是否使用 toy 数据、以及是否已训练足够步数。  
> toy 数据用于通路验证，不建议用于生成质量判断。
...
# 诊断模式（用于定位 CUDA / 数值问题）
python train.py --scale 0.1b --diag baseline
python train.py --scale 0.1b --diag no_amp
python train.py --scale 0.1b --diag fp32_kernel
